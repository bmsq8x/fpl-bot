...
def fetch_manager_squad(manager_id):
  try:
    ...
    mgr_res = requests.get(
        f"https://fantasy.premierleague.com/api/entry/{manager_id}/", timeout=10
    )
    mgr_info = ""
    if mgr_res.status_code == 200:
      mdata = mgr_res.json()
      mgr_info = (
          f"اسم الفريق: {mdata.get('name')} | المدرب:"
          f" {mdata.get('player_first_name')} {mdata.get('player_last_name')} |"
          f" النقاط الكلية: {mdata.get('summary_overall_points')} | الترتيب"
          f" العام: {mdata.get('summary_overall_rank')}"
      )
    ...
    if picks_res.status_code != 200:
      ...
      if picks_res.status_code != 200:
        return (
            None,
            None,
            f"لم يتم العثور على تشكيلة مجهزة للمعرف {manager_id}.",
        )
    ...
    for p in picks:
      ...
      line = (
          f"- {pname} ({tname}) | المركز: {ptype} | السعر: £{cost}M{role_str}"
      )
      card_html = (
          f"<div class='player-card'>{pname}{role_str}<span>{tname}"
          f" (£{cost}M)</span></div>"
      )
  ...
  try:
    ...
    if res.status_code == 200:
      ...
      for m in standings[:10]:
        top_managers.append(
            f"- المرتبة {m.get('rank')}: {m.get('entry_name')}"
            f" ({m.get('player_name')}) | النقاط الكلية: {m.get('total')} | (ID"
            f" المدرب: {m.get('entry')})"
        )

      summary = (
          f"=== بيانات دوري: {league_name} (League ID: {league_id}) ===\n"
          + "\n".join(top_managers)
      )
      return summary, None
  except Exception as e:
...
def fetch_fixtures_fdr():
  try:
    res = requests.get(
        "https://fantasy.premierleague.com/api/fixtures/?future=1", timeout=10
    )
    static_res = requests.get(
        "https://fantasy.premierleague.com/api/bootstrap-static/", timeout=10
    )
    if res.status_code == 200 and static_res.status_code == 200:
      ...
      for fix in fixtures[:30]:
        ...
        upcoming_summary.append(
            f"الجولة {gw}: {h_team} (صعوبة {h_diff}) ضد {a_team} (صعوبة"
            f" {a_diff})"
        )
    ...
    pass
  return (
      "تعذر جلب الجدول المباشر، سيتم الاعتماد على التحليل المباشر للنموذج."
  )
...
def fetch_live_fpl_data():
  try:
    ...
    if res.status_code == 200:
      ...
      for p in players:
        if p.get("news"):
          ...
          news_list.append(
              f"- {p['web_name']} ({team_name}): {p['news']} (نسبة الجاهزية:"
              f" {p.get('chance_of_playing_next_round', 100)}%)"
          )

      sorted_by_transfers = sorted(
          players, key=lambda x: x.get("transfers_in_event", 0), reverse=True
      )[:15]
      trending_players = [
          f"- {p['web_name']} ({teams.get(p['team'], '')}): ملكية"
          f" {p.get('selected_by_percent', '0')}% | عمليات الشراء مؤخراً:"
          f" {p.get('transfers_in_event', 0)}"
    ...
    pass
  return (
      "تحديثات المؤتمرات وتوجهات الدرافتات الحية تعتمد على التحليل المباشر"
      " للنموذج."
  )


# ---------------------------------------------------------
# 4. توجيهات الذكاء الاصطناعي والدعم المباشر الديناميكي
# ---------------------------------------------------------
...
def run_fpl_ai(api_key, prompt, images=None):
  if not api_key:
    raise Exception(
        "مفتاح Gemini API غير مدخل. يرجى إدخال المفتاح في الشريط الجانبي."
    )

  genai.configure(api_key=api_key)
  live_context = fetch_live_fpl_data()
  augmented_prompt = (
      f"{SYSTEM_INSTRUCTION}\n\n[البيانات والمستجدات الحية والدرافتات المحدثة"
      f" خلال هذه الساعة]:\n{live_context}\n\n[طلب المستخدم]:\n{prompt}"
  )

  contents = [augmented_prompt]

  # تحويل الصور المرفوعة لتنسيق RGB متوافق
  if images:
    img_list = images if isinstance(images, list) else [images]
    for img in img_list:
      if isinstance(img, Image.Image):
        contents.append(img.convert("RGB"))

  # استكشاف الموديلات المتاحة تلقائياً لتفادي خطأ 404 Not Found
  candidate_models = []
  try:
  ...
  # إضافة خيارات احتياطية مع السابقة
  fallback_models = [
  ...
  raise Exception(f"تعذر الاتصال بـ Gemini API: {last_err}")
...
with st.sidebar:
  ...
  gemini_key_input = st.text_input(
      "مفتاح Gemini API:", value=secrets_gemini, type="password"
  )
  gemini_key = gemini_key_input if gemini_key_input else secrets_gemini

  st.markdown("---")
  st.header("🔗 ربط حساب الفانتسي")
  saved_fpl_id_input = st.text_input(
      "رقم معرف فريقك (FPL Team ID):",
  ...
  st.markdown("---")
  st.success("⚡ التحديث التلقائي واكتشاف الموديلات المباشر مُفعّل")
...
st.markdown(
    "<h1 style='text-align: center; color: #FF1A1A; font-size: 42px; font-weight:"
    " bold; margin-bottom: 10px;'>( حميدي مطلق ما تفوز علي )</h1>",
    unsafe_allow_html=True,
)

st.title("⚽ بوت الفانتسي الذكي المباشر - موسم 2026/2027")
st.caption(
    "تحديث تلقائي كل ساعة للأخبار، الإصابات، المؤتمرات، والدرافتات الحديثة"
)
st.markdown("---")
...
if gemini_key:
  ...
  if page == "📊 تحليل التشكيلة والتقرير اليومي (مع الملعب والواتساب)":
    ...
    input_method = st.radio(
        "اختر طريقة إدخال التشكيلة:",
        ["🔗 جلب تلقائي عبر ID الفريق", "📸 رفع لقطة شاشة (صورة)"],
    )

    if input_method == "🔗 جلب تلقائي عبر ID الفريق":
      fpl_id_input = st.text_input(
          "ادخل رقم معرف فريقك (FPL Team ID):",
          value=saved_fpl_id if saved_fpl_id else "",
      )
      if st.button("🚀 جلب وتحليل التشكيلة بـ ID"):
        if fpl_id_input:
          with st.spinner(
              "جاري جلب تشكيلتك المباشرة ورسمها على الملعب..."
          ):
      ...
      with col1:
        uploaded_file = st.file_uploader(
            "📸 ارفع تشكليتك الحالية لموسم 2026/27",
            type=["png", "jpg", "jpeg"],
        )
      if uploaded_file:
        ...
        if st.button("🚀 بدء التحليل المباشر من الصورة"):
          prompt = (
              "اقرأ التشكيلة من الصورة وقدم تقريراً شاملاً بالتقييم"
              " والغيابات، وأضف في النهاية ملخصاً للواتساب مع إيموجيات."
          )
          try:
  ...
  # 2. رادار المداورة xMins
    ...
    st.write(
        "تحليل احتمالية تدوير اللاعبين والدكة بناءً على ضغط المباريات الأوروبية"
        " والمؤتمرات:"
    )
  ...
  # 3. مخطط الخصائص Chips Planner
    ...
    st.write(
        "تحديد الجولات المضاعفة (Double GW) والفارغة (Blank GW) والتوقيت المثالي"
        " لاستخدام الخصائص:"
    )
  ...
  # 4. مؤشر الملكية المؤثرة EO Risk
    ...
    st.header(
        "🛡️ مؤشر الملكية المؤثرة وتقييم مخاطر السوق (Effective Ownership)"
    )
    st.write(
        "تقييم مدى الضرر المتوقع على ترتيبك في حال عدم امتلاكك للاعبين الأكثر"
        " شعبية:"
    )
  ...
  # 5. حاسبة الدوري الخاص
    ...
    league_id_in = st.text_input(
        "🔢 ادخل رقم ID دوريك الخاص (Private League ID):",
        placeholder="مثال: 123456",
    )
    ...
    if st.button("🔍 تحليل الدوري الخاص وصدارة القائمة"):
      if league_id_in:
        with st.spinner(
            "جاري سحب ترتيب الدوري الخاص وتحليل صدارة المنافسين..."
        ):
  ...
  # 6. مخطط التبديلات للجولات القادمة
    ...
    st.write(
        "يساعدك هذا القسم على توزيع تبديلاتك المجانية لـ 3 جولات قادمة بناءً على"
        " صعوبة المباريات (FDR):"
    )

    target_id = st.text_input(
        "ادخل FPL ID لتخطيط فريقك:", value=saved_fpl_id if saved_fpl_id else ""
    )
  ...
  # 7. رادار الدرافتات
    ...
    pos_filter = st.selectbox(
        "🎯 تصفية حسب المركز:",
        ["الكل", "حراس المرمى", "المدافعون", "خط الوسط", "المهاجمون"],
    )
    if st.button("🔍 كشف أكثر اللاعبين تكراراً في الدرافتات الأخيرة"):
      draft_prompt = (
          "قدم تحليلاً دقيقاً لأكثر اللاعبين تكراراً واختياراً في الدرافتات"
          " خلال الـ 3 أيام الماضية لموسم 2026/2027. (التصفية:"
          f" {pos_filter})"
      )
      try:
  ...
  # 8. رادار المؤتمرات
    ...
    if st.button("🔄 جلب ملخص المؤتمرات الصحفية للجولة"):
      press_prompt = (
          "قدم ملخصاً تحليلياً لأهم ما ورد في المؤتمرات الصحفية لمدربي"
          f" البريميرليج هذا الأسبوع لموسم 2026/2027. {manager_query}"
      )
      try:
        with st.spinner("جاري التحليل..."):
    ...
    with wc1:
      ...
      strategy = st.selectbox(
          ...
          [
              "متوازنة",
              "هجوم ناري",
              "دفاع صلب برخص بالوسط",
              "مغامرة Differential",
          ],
      )
    with wc2:
      target_gws = st.slider(
          "🗓️ التخطيط للجولات القادمة:", min_value=3, max_value=8, value=5
      )

    if st.button("✨ بناء التشكيلة المثالية"):
      wc_prompt = (
          f"قم ببناء تشكيلة Wildcard كاملة (15 لاعباً) بميزانية {budget}M"
          f" بأسلوب '{strategy}' للجولات الـ {target_gws} القادمة."
      )
      try:
    ...
    if st.button("🔄 جلب أحدث تغيرات الأسعار"):
      try:
        with st.spinner("جاري قراءة البيانات الحية..."):
          res_text, _ = run_fpl_ai(
              gemini_key,
              "قدم تقريراً شاملاً بجداول عن أحدث تغيرات الأسعار واللاعبين القريبين"
  ...
  # 11. المساعد الصوتي والدردشة
    ...
    if p := st.chat_input(
        "اسأل عن أي درافت، مؤتمر صحفي، لاعب، أو تبديل اليوم..."
    ):
  ...
  # 12. مقارنة الخصم
    ...
    c1, c2 = st.columns(2)
    with c1:
      f_m = st.file_uploader(
          "📸 تشكليتك", type=["png", "jpg", "jpeg"], key="m"
      )
    with c2:
      f_r = st.file_uploader(
          "📸 تشكيلة الخصم", type=["png", "jpg", "jpeg"], key="r"
      )
      ...
      if st.button("🔍 تحليل الفروق ومصدر النقاط"):
        try:
          with st.spinner("جاري مقارنة التشكيلتين..."):
            res_text, _ = run_fpl_ai(
                gemini_key,
                "قارن بين التشكيلتين واكشف نقاط التفوق لكل فريق.",
                images=[Image.open(f_m), Image.open(f_r)],
            )
            st.markdown(res_text)
        except Exception as e:
          st.error(f"خطأ: {e}")
  ...
  # 13. مصفوفة الكابتن
    ...
    gw_num = st.number_input(
        "🎯 رقم الجولة:", min_value=1, max_value=38, value=1
    )
    if st.button("⚡ ترشيح الكابتن"):
      try:
        with st.spinner("جاري تحليل خيارات الكابتن..."):
          res_text, _ = run_fpl_ai(
              gemini_key, f"قدم أفضل 3 خيارات كابتن للجولة {gw_num}."
          )
          st.markdown(res_text)
      except Exception as e:
        st.error(f"خطأ: {e}")
  ...
  # 14. استراتيجيات المغامرة
    ...
    strat_choice = st.radio(
        "اختر نوع الريسك:",
        [
            "خصم نقاط للتبديلات (-4/-8)",
            "حرق الميزانية هجومياً",
            "كابتن Differential",
            "توقيت الخصائص (Chips)",
        ],
    )
    if st.button("🤖 طلب تقييم المغامرة"):
      try:
        with st.spinner("جاري دراسة نسبة المخاطرة..."):
          res_text, _ = run_fpl_ai(
              gemini_key,
              f"قيم استراتيجية الريسك التالية هذا الأسبوع: {strat_choice}",
          )
          st.markdown(res_text)
      except Exception as e:
        st.error(f"خطأ: {e}")
      ...
      if user_log:
        try:
          with st.spinner("جاري مراجعة الأداء..."):
            res_text, _ = run_fpl_ai(
                gemini_key,
                "قيم قرارات المدرب التالية واذكر الإيجابيات والسلبيات:"
                f" {user_log}",
            )
            st.markdown(res_text)
        except Exception as e:
