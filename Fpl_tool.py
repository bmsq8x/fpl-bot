...
# ---------------------------------------------------------
# 4. الاتصال المباشر بـ OpenAI
# ---------------------------------------------------------
SYSTEM_PROMPT = """
أنت مدير ومنصة الذكاء الاصطناعي الاحترافية BMS bot FPL 26/27 لموسم 2026/2027.
تعتمد تحليلاتك حصرياً على البيانات الفعلية والدقيقة الواردة من FPL API.
"""
...
if category == "🏠 لوحة التحكم الرئيسية والدوريات":
  ...
  expert_tweets = st.text_area(
      "📥 لصق تغريدات الخبراء (اختياري):", height=80
  )
  if not user_fpl_id:
    ...
    if st.button("🚀 جلب وتحليل التشكيلة الحية"):
      ...
      if err:
        ...
        st.session_state["cached_analysis"] = ask_openai(
            f"حلل هذه التشكيلة: [{squad_txt}]", extra_context=expert_tweets
        )
  ...
  st.header("🔄 قسم التبديلات الذكية (مطابقة المراكز الصارمة)")
  if not user_fpl_id:
    st.warning("⚠️ أدخل رقم FPL Team ID في القائمة الجانبية.")
  else:
    if st.button("🔍 حساب أفضل التبديلات المتاحة"):
      ...
      if err:
        ...
        res = ask_openai(
            f"اقترح تبديلين حصرياً في نفس المركز للتشكيلة: [{squad_txt}]"
        )
        st.markdown(res)

elif category == "👑 مصفوفة الكابتن الاستراتيجية":
  st.header("👑 أفضل خيارات الكابتن")
  if st.button("🚀 تحليل خيارات الكابتن"):
    st.markdown(
        ask_openai(
            "من هم أفضل 3 مرشحين لشارة الكابتن بناءً على صعوبة المباريات FDR؟"
        )
    )
